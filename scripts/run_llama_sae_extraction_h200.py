#!/usr/bin/env python3
"""
H200-optimized Llama-3.1-8B SAE extraction.

Designed for ~140 GB VRAM. Loads the model and ALL 32 SAEs for a variant at
once, then processes the corpus in one forward pass with hooks on every MLP.
No CPU round-trips, no per-layer chunking, no repeated SAE loads.

Uses OpenMOSS Llama-Scope SAEs:
  "32x" -> llama_scope_lxm_8x  (32K features, ~8x expansion)
  "64x" -> llama_scope_lxm_32x (128K features, ~32x expansion)
...
def _load_all_saes(variant: str, device: str, dtype):
...
def _encode_pass(model, tokenizer, saes: dict, corpus, group_fn, n_groups, batch_size, max_length, device, desc: str):
    import torch
...
def run_variant(variant: str, batch_size: int = 128, max_length: int = 256):
    ...
    saes = _load_all_saes(variant, device, dtype)
