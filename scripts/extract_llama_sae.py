#!/usr/bin/env python3
"""
Extract SAE features for meta-llama/Llama-3.1-8B MLP layers using EleutherAI SAEs.

Downloads corpora + SAE weights from HuggingFace inside the Modal job.

Run:
    cd ~/GWIQ-atlas
    modal run scripts/extract_llama_sae.py

After it finishes, inside the shell:
...
app = modal.App("llama-sae-extract")

GWIQ_OUTPUT_VOLUME_NAME = "gwiq-atlas-output"
GWIQ_OUTPUT_MOUNT = "/gwiq-output"
...
image = (
    ...
    .pip_install(
        ...
        "accelerate",
        "bitsandbytes>=0.45.0",
        "qwip-atlas[analysis] @ git+https://github.com/JuiceB0xC0de/GWIQ-atlas.git",
        "sae @ git+https://github.com/EleutherAI/sae.git",
    )
)

gwiq_output_volume = modal.Volume.from_name(GWIQ_OUTPUT_VOLUME_NAME, create_if_missing=True)
...
def _run_variant(variant: str, batch_size: int, max_length: int, token: str | None):
    import torch
    import numpy as np
    ...
    outdir = Path(GWIQ_OUTPUT_MOUNT) / "llama-3-8b" / "sae"
    outdir.mkdir(parents=True, exist_ok=True)
    N_topic = float(t_n.sum().item())
    d_sae = next(iter(saes.values())).w_enc.shape[1]
    ...
    for L in sorted(saes.keys()):
        ...
        out_path = outdir / f"sae_l{L}_{variant}.npz"
        np.savez_compressed(
            out_path,
...
@app.function(
    ...
    cpu=30,
    timeout=86400,
    volumes={GWIQ_OUTPUT_MOUNT: gwiq_output_volume},
    secrets=[modal.Secret.from_name("huggingface")],
)
def extract_both(batch_size: int = 16, max_length: int = 256):
    token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGING_FACE_HUB_TOKEN")
    results = {}
    for variant in ["32x", "64x"]:
        results[variant] = _run_variant(variant, batch_size, max_length, token)
    return results


@app.local_entrypoint()
def main(batch_size: int = 16, max_length: int = 256):
    print(extract_both.remote(batch_size, max_length))
    print("\nNext inside shell:")
    print("  qwip-build-atlas --atlas /gwiq-output/atlas index")


if __name__ == "__main__":
    print("Run with: cd ~/GWIQ-atlas && modal run scripts/extract_llama_sae.py")
