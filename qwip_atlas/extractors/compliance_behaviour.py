...
def _batch_last_token_components(captured: dict[tuple[int, str], Any], layer: int, info: dict):
    ...
    # With left-padding, the last token of the actual sequence is always at index -1.
    # Slice the tensors before applying activation function or reshaping.
    tensors = {
        "mlp": mlp_hidden[:, -1, :],
        "up": captured.get((layer, "up")),
        "attn": captured.get((layer, "attn_out")),
    }
    if tensors["up"] is not None:
        tensors["up"] = tensors["up"][:, -1, :]
    if tensors["attn"] is not None:
        tensors["attn"] = tensors["attn"][:, -1, :]

    gate_pre = captured.get((layer, "gate_pre"))
    tensors["gate"] = act_fn(gate_pre[:, -1, :]) if gate_pre is not None else None

    for k in ["attn_pre", "q", "k", "v"]:
        t = captured.get((layer, k))
        if t is not None:
            name = k if k != "attn_pre" else "heads"
            res = _safe_per_head(t[:, -1, :].unsqueeze(1), head_dim)
            tensors[name] = res.squeeze(1) if res is not None else None
    ...
    for start in tqdm(range(0, len(corpus), cfg.batch_size), desc="batches", unit="batch"):
        ...
        for layer, info in per_layer_info.items():
            ...
            for batch_idx, ((_, label), _) in enumerate(zip(batch, seq_lens)):
                for comp, batch_vectors in batch_comps.items():
                    if comp in cfg.components:
                        values[layer][comp][label].append(batch_vectors[batch_idx])
        captured.clear()
