...
def _last_token_components(captured: dict[tuple[int, str], Any], layer: int, info: dict):
    import torch
    ...
    # Slice to the last token BEFORE applying activation to save massive computation
    gate_post = act_fn(gate_pre[:, -1:]) if gate_pre is not None else None

    def _get_last(comp_key):
        t = captured.get((layer, comp_key))
    ...
    out = {}
    # Assume all batch items share the same shape
    batch_size = mlp_hidden.shape[0]
    for batch_idx in range(batch_size):
        item_out = {}
        for name, tensor in tensors.items():
            if tensor is None:
                continue
            item_out[name] = tensor[batch_idx].reshape(-1).cpu().float().numpy()
        out[batch_idx] = item_out
    return out
...
def run_compliance_behaviour(cfg: ComplianceBehaviourRunConfig, hf_token: str | None = None) -> dict:
    ...
    for start in tqdm(range(0, len(corpus), cfg.batch_size), desc="batches", unit="batch"):
        ...
        for layer, info in per_layer_info.items():
            comps_batch = _last_token_components(captured, layer, info)
            for batch_idx, ((_, label), _) in enumerate(zip(batch, seq_lens)):
                comps = comps_batch.get(batch_idx, {})
