...
def _safe_per_head(tensor, head_dim: int | None):
    if tensor is None or not head_dim or tensor.shape[-1] % head_dim != 0:
        return None
    # For a token slice (e.g., shape [B, D]), the reshape is different than for full sequence
...
def _batch_last_token_components(captured: dict[tuple[int, str], Any], layer: int, info: dict):
    import torch
    ...
    # Slicing [:, -1] before applying act_fn prevents computing activations
    # on padded tokens or history tokens that are ultimately discarded.
    gate_pre = captured.get((layer, "gate_pre"))
    gate_post = act_fn(gate_pre[:, -1]) if gate_pre is not None else None
    tensors = {
        "mlp": mlp_hidden[:, -1] if mlp_hidden is not None else None,
        "gate": gate_post,
        "up": captured.get((layer, "up"))[:, -1] if captured.get((layer, "up")) is not None else None,
        "attn": captured.get((layer, "attn_out"))[:, -1] if captured.get((layer, "attn_out")) is not None else None,
        "heads": _safe_per_head(captured.get((layer, "attn_pre"))[:, -1] if captured.get((layer, "attn_pre")) is not None else None, head_dim),
        "q": _safe_per_head(captured.get((layer, "q"))[:, -1] if captured.get((layer, "q")) is not None else None, head_dim),
        "k": _safe_per_head(captured.get((layer, "k"))[:, -1] if captured.get((layer, "k")) is not None else None, head_dim),
        "v": _safe_per_head(captured.get((layer, "v"))[:, -1] if captured.get((layer, "v")) is not None else None, head_dim),
    }
    ...
    for name, tensor in tensors.items():
        if tensor is None:
            continue
        # Reshape to flatten head dimension if it was added, otherwise just flatten the last dim
        if len(tensor.shape) > 2:
            tensor = tensor.reshape(tensor.shape[0], -1)
        out[name] = tensor.cpu().float().numpy()
    return out
...
def run_compliance_behaviour(cfg: ComplianceBehaviourRunConfig, hf_token: str | None = None) -> dict:
    ...
    for start in tqdm(range(0, len(corpus), cfg.batch_size), desc="batches", unit="batch"):
        ...
        for layer, info in per_layer_info.items():
            batch_comps = _batch_last_token_components(captured, layer, info)
            for batch_idx, ((_, label), _) in enumerate(zip(batch, seq_lens)):
                for comp, batch_vector in batch_comps.items():
                    if comp in cfg.components:
                        values[layer][comp][label].append(batch_vector[batch_idx])
        captured.clear()
