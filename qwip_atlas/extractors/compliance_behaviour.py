...
def _extract_batch_components(captured: dict[tuple[int, str], Any], layer: int, info: dict, components: set[str]):
    import torch
    ...
    tensors = {"mlp": get_last_token("mlp_hidden")}

    if "gate" in components:
        gate_pre = get_last_token("gate_pre")
        tensors["gate"] = act_fn(gate_pre) if gate_pre is not None else None

    if "up" in components:
        tensors["up"] = get_last_token("up")

    if "attn" in components:
        tensors["attn"] = get_last_token("attn_out")

    if "heads" in components:
        tensors["heads"] = _safe_per_head(get_last_token("attn_pre"), head_dim)

    if "q" in components:
        tensors["q"] = _safe_per_head(get_last_token("q"), head_dim)

    if "k" in components:
        tensors["k"] = _safe_per_head(get_last_token("k"), head_dim)

    if "v" in components:
        tensors["v"] = _safe_per_head(get_last_token("v"), head_dim)
...
def run_compliance_behaviour(cfg: ComplianceBehaviourRunConfig, hf_token: str | None = None) -> dict:
    ...
    for start in tqdm(range(0, len(corpus), cfg.batch_size), desc="batches", unit="batch"):
        ...
        for layer, info in per_layer_info.items():
            batch_comps = _extract_batch_components(captured, layer, info, cfg.components)
