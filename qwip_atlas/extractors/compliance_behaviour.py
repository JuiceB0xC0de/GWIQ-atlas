...
def _last_batch_components(captured: dict[tuple[int, str], Any], layer: int, info: dict):
    import torch
    ...
    # Slice to last token before expensive operations to save O(B*S*D) calculations
    # tokenizer.padding_side = 'left', so last token is always at index -1
    last_tokens = {}
    for key in ["mlp_hidden", "gate_pre", "up", "attn_out", "attn_pre", "q", "k", "v"]:
        tensor = captured.get((layer, key))
        if tensor is not None:
            last_tokens[key] = tensor[:, -1, :]

    gate_post = act_fn(last_tokens["gate_pre"]) if "gate_pre" in last_tokens else None
    tensors = {
        "mlp": last_tokens.get("mlp_hidden"),
        "gate": gate_post,
        "up": last_tokens.get("up"),
        "attn": last_tokens.get("attn_out"),
        "heads": _safe_per_head(last_tokens.get("attn_pre"), head_dim),
        "q": _safe_per_head(last_tokens.get("q"), head_dim),
        "k": _safe_per_head(last_tokens.get("k"), head_dim),
        "v": _safe_per_head(last_tokens.get("v"), head_dim),
    }
    ...
    for name, tensor in tensors.items():
        if tensor is None:
            continue
        # tensor is [B, ...] so reshape each item to flat vector [B, -1]
...
def run_compliance_behaviour(cfg: ComplianceBehaviourRunConfig, hf_token: str | None = None) -> dict:
    ...
    for start in tqdm(range(0, len(corpus), cfg.batch_size), desc="batches", unit="batch"):
        ...
        for layer, info in per_layer_info.items():
            batch_comps = _last_batch_components(captured, layer, info)
