...
def _last_token_components_batch(captured: dict[tuple[int, str], Any], layer: int, info: dict):
    import torch
    ...
    # Left padding means the actual last token of sequence is always at index -1
    # We slice to the last token *before* expensive operations like act_fn
    def get_last(t):
        if t is None:
            return None
        return t[:, -1, :]

    gate_pre_last = get_last(captured.get((layer, "gate_pre")))
    gate_post_last = act_fn(gate_pre_last) if gate_pre_last is not None else None

    tensors = {
        "mlp": get_last(mlp_hidden),
        "gate": gate_post_last,
        "up": get_last(captured.get((layer, "up"))),
        "attn": get_last(captured.get((layer, "attn_out"))),
        "heads": _safe_per_head(get_last(captured.get((layer, "attn_pre"))), head_dim),
        "q": _safe_per_head(get_last(captured.get((layer, "q"))), head_dim),
        "k": _safe_per_head(get_last(captured.get((layer, "k"))), head_dim),
        "v": _safe_per_head(get_last(captured.get((layer, "v"))), head_dim),
    }
...
def run_compliance_behaviour(cfg: ComplianceBehaviourRunConfig, hf_token: str | None = None) -> dict:
    ...
    for start in tqdm(range(0, len(corpus), cfg.batch_size), desc="batches", unit="batch"):
        ...
        for layer, info in per_layer_info.items():
            batch_comps = _last_token_components_batch(captured, layer, info)
